...
public sealed class AspireFixture : IAsyncLifetime
{
        ...
        tokenManagementServices
           ...
           .AddClient(
                ...
                client =>
                {
                    ...
                    client.TokenEndpoint = new Uri(identityServerUri, "/connect/token");
                    client.ClientId = ClientId.Parse("integration-tests");
                    client.ClientSecret = ClientSecret.Parse("password");
    ...
    public async Task<HttpClient> CreateAuthenticatedHttpClientAsync(string resourceName)
    {
        ...
        var result = await TokenManagementServiceProvider
           .GetRequiredService<IClientCredentialsTokenManager>()
           .GetAccessTokenAsync(ClientCredentialsClientName.Parse("integration-tests"));
        if (result.Token is null)
        {
            throw new AuthenticationException("Could not authenticate integration test with Client Credentials");
        }

        httpClient.SetBearerToken(result.Token.AccessToken);
        return httpClient;
    }
